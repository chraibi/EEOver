...
#if DEBUG
     printf("Enter ellipse_ellipse_overlap_gsl\n");
#endif
...
//-- determine which roots are real, discard any complex roots
     ...
     for (i = 1; i <= nroots; i++){ 
         /* for (i = nroots; i >= 1; i--){ */
          duplicate = 0;
          if (fabs (r[2][i]) < EPS){
               //check if duplicate aka multiple root
//----------------------------------
               for(j=nychk-1;j>=0;--j){
                    /* printf("i=%d, j=%d, ychk[%d]=%f\n",i,j,j,ychk[j]); */
                    if(  fabs (r[1][i]*B1- ychk[j]) < EPS/2.0 )
                         duplicate=1;
               }
//-----------------------------------
...
// However in this case the roots alternate! (^^_^^)
/* cy[0]=64.000000 */
/* cy[1]=0.000000 */
/* cy[2]=-128.000000 */
/* cy[3]=0.000000 */
...
/* py[4]=1.000000 */
/* nroots=4 */
/* r[1][1]=-0.9999999903, r[2][1]=0.0000000000 */
/* ------->eval_complex=[0.000000, 0.000000] */
/* r[1][2]=0.9999999904, r[2][2]=0.0000000000 */
/* ------->eval_complex=[0.000000, 0.000000] */
/* r[1][3]=-1.0000000097, r[2][3]=0.0000000000 */
/* ------->eval_complex=[0.000000, 0.000000] */
/* r[1][4]=1.0000000096, r[2][4]=0.0000000000 */
/* ------->eval_complex=[0.000000, 0.000000] */
...
//-- for the two ellipses
     ...
     for (i = 0; i < nychk; i++){
          ...
          if (fabs(ellipse2tr(x1, ychk[i], AA, BB, CC, DD, EE, FF)) < EPS){
               nintpts++;
#if DEBUG
               printf("Intersection point. first if x1. acc nintps=%d\n",nintpts);
#endif
               if (nintpts > 4){
...
#if DEBUG
               printf("X1: nintpts=%d, xint=%f, x2=%f, i=%d, yint=%f\n",nintpts, x1, x2, i, ychk[i]);
#endif
          }
          ...
          if (fabs(x2-x1)>0.001 && (fabs(ellipse2tr(x2, ychk[i], AA, BB, CC, DD, EE, FF)) < EPS)
               ...
               yint[nintpts-1] = ychk[i];
#if DEBUG
               printf("X2: nintpts=%d, x1=%f, xint=%f, i=%d, yint=%f\n",nintpts, x1, x2, i, ychk[i]);
#endif
          }
     }
...
{
#if DEBUG
     printf("Enter ellipse_ellipse_overlap_gems\n");
#endif
